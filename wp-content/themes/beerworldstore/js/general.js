...
;(function($, window, document, undefined) {
	...
	var $screensize = function(element){  
		...
		$win.on('resize', function(){
			if (screencheck(767)){
				$('.nothingfound-sidebar').removeClass('sticky')
			} else {
				$('.nothingfound-sidebar').addClass('sticky')
			}
		});
		/* Tab Content box 
		---------------------------------------------------------------------*/
		var tabBlockElement = $('.tab-data');
			$(tabBlockElement).each(function() {
				...
				jQuery(".page-template-beer-holidays .tab-data").find(".tabnav li").first().removeClass("active");
				jQuery(".page-template-beer-holidays .tab-data").find(".tabcontent").first().hide();
		...
		* ----------------------------------------------------------------------------------------------------------------------*/
		...
		$('.ac-formbottomarea .form-group').hover(function() {
			var paddingWidth = $(this).find('label').width();
			$(this).find('input').css('padding-left', paddingWidth + 28 + 'px');
		})

		$('.ac-formbottomarea .form-group').focus(function() {
			var paddingWidth = $(this).find('label').width();
			$(this).find('input').css('padding-left', paddingWidth + 28 + 'px');
		})
		...
		$('.ac-formbottomarea .form-group').hover(function() {
			var paddingWidth = $(this).find('label').width();
			$(this).find('textarea').css('padding-left', paddingWidth + 28 + 'px');
		})
		...
		// $('.custom-select label').each(function() {
		// 	var paddingWidth = $(this).width();
		// 	$('.custom-select').find('.select2-selection').css('padding-left', paddingWidth + 28 + 'px');
		// })
		
		// $(document).on('hover', '.custom-select', function(e){
		// 	var paddingWidth = $(this).find('label').width();
		...
		$(window).load(function(){
			$( ".custom-select" ).each(function( index ) {
				var paddingWidth = $(this).find('label').width();
				$(this).find('.select2-selection__rendered').css('padding-left', paddingWidth + 28 + 'px');
			});
		 });



		// $(window).on('load', '.custom-select', function(e){
		// 	alert();
			// var paddingWidth = $(this).find('label').width();
			// $('.custom-select').find('.select2-selection__rendered').css('padding-left', paddingWidth + 28 + 'px');
		// })		
		...
		// $(document).on('hover', '.custom-select', '#addMoreBeer', function(e){
		// 	var paddingWidth = $(this).find('label').width();
		// 	$('.custom-select').find('.select2-selection__rendered').css('padding-left', paddingWidth + 28 + 'px');
		// })
		
		// $(document).on('click', '#addMoreBeer', function(e){
		...
		$dialogTrigger.click( function(){
			...
			jQuery(".beerdetail-area").html(jQuery(this).next(".showPopupDetails").html());
